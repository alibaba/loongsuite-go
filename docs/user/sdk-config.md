...
- `OTEL_INSTRUMENTATION_HTTP_EXCLUDE_PATHS`: Specifies a comma-separated list of URL paths to exclude from HTTP auto-instrumentation (e.g., `/ping,/health,/metrics`). Requests matching these paths will not generate spans. By default, no paths are excluded.
- `OTEL_INSTRUMENTATION_HTTP_EXCLUDE_PATH_REGEX`: Specifies a comma-separated list of regular expression patterns to exclude URL paths from HTTP auto-instrumentation (e.g., `^/api/v[0-9]+/internal/.*,^/health$`). Requests whose paths match any of these patterns will not generate spans. Can be used alongside `OTEL_INSTRUMENTATION_HTTP_EXCLUDE_PATHS` for both exact and pattern-based exclusion. By default, no patterns are configured.
